...
BEGIN
	...
	OPEN p_parents FOR
		...
		max_rank_strategy_id AS (
			...
			WHERE  strategy_rank = (SELECT MAX(strategy_rank) FROM proportioned_generation)
		),
		
		-- fitness proportion limits of each strategy in current generation
		proportion_limits AS (
			SELECT pg_b.strategy_id,
		...
		random_parent_numbers AS (
			SELECT CASE WHEN DBMS_RANDOM.VALUE < v_crossover_rate THEN 1 ELSE 0 END perform_crossover,
