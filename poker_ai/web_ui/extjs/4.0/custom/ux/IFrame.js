 ...
 *
 * From Ext JS 4.2.1 ([original documentation][1])
 *   [1]: http://docs.sencha.com/extjs/4.2.1/#!/api/Ext.ux.IFrame
 */
...
/**
 * SMS extensions to {@link Ext.ux.IFrame}.
 */
Ext.define("Sms.ux.IFrame", {
	override: "Ext.ux.IFrame",

	src: "blank.jsp",

	/**
	 * @inheritdoc Ext.ux.IFrame#onRelayedEvent
	 * @localdoc Supports the newer {@code Ext.Element.getTrueXY} method if it
	 * exists, or defaults to original behavior.
	 */
	onRelayedEvent: function(event){
		var iframeEl        = this.iframeEl;
		var originalEventXY = event.getXY();
		var iframeXY,
			eventXY;

		if(Ext.Element.getTrueXY){
			iframeXY = Ext.Element.getTrueXY(iframeEl);
			eventXY  = Ext.EventManager.getPageXY(event.browserEvent);
		}
		else{
			iframeXY = iframeEl.getXY();
			eventXY  = originalEventXY;
		}
