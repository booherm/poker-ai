...
Ext.define('Sch.patches.CellEditing', {
    ...
    overrides: {
        // PATCH for broken Ext cell editing in locked grid
//        getEditingContext: function (record, columnHeader) {
//            var parent = this.grid.up();

//            var usePatched = parent && (Sch.panel.TimelineGridPanel && parent instanceof Sch.panel.TimelineGridPanel ||
//                                        Sch.panel.TimelineTreePanel && parent instanceof Sch.panel.TimelineTreePanel);
//            
//            if (!usePatched) {
//                return this.callOverridden(arguments);
//            }

//            // At this point we assume editing is happening in the locked grid
...
Ext.define('Sch.patches.GroupingFeature', {
    ...
    // Overridden to be able to pass group name as one of the fireEvent arguments (needed to have locked+normal groups in sync)
        expand: function(group, preventSizeCalculation) {
            var me = this,
            view = me.view,
            ...
            var groupName = groupBdDom.id.match(view.id + '-gp-(.*)')[1];
            view.fireEvent('groupexpand', view, groupName);
        },

    
        collapse: function(group,  preventSizeCalculation) {
            var me = this,
                view = me.view,
                grid = view.up('gridpanel'),
                groupBdDom;
...
This class represent a model with customizable field names. 
...
Ext.define('Sch.model.Customizable', {
    ...
    onClassExtended : function (cls, data, hooks) {
        ...
        hooks.onBeforeCreated = function (cls, data) {
            ...
            if (!proto._defaultFields_) {
                return;
            }

            proto._defaultFields_   = proto._defaultFields_.concat(cls.superclass._defaultFields_ || []);
            
            var defaultFields       = proto._defaultFields_;
            
            // already processed by the Ext.data.Model `onBeforeCreated`
            var fields              = proto.fields;
            
            Ext.Array.each(defaultFields, function (defaultField, index) {
                // converting the items of the _defaultFields_ array to objects with "isDefaultField" property to be able identify them later 
                // not touching the array itself, since its going to be inhertied through the prototype chain
                if (typeof defaultField == 'string') {
                    defaultFields[ index ]          = defaultField = { name : defaultField };
                    
                    defaultField.isDefaultField     = true;
                }
                
                var defaultFieldName    = defaultField.name;
                
                var fieldProperty       = defaultFieldName.charAt(0).toLowerCase() + defaultFieldName.substr(1) + 'Field';
                var overrideFieldName   = proto[ fieldProperty ];
                
                if (overrideFieldName && overrideFieldName != defaultFieldName) {
                    if (!fields.getByKey(overrideFieldName)) {
                        throw new Error("The override field [" + overrideFieldName + "] is not provided in definition of " + cls.$className);
                    }
                    
                    // remove the default fields inherited from the base model, in case we are overrding them
                    if (fields.getByKey(defaultFieldName) && fields.getByKey(defaultFieldName).isDefaultField) {
                        fields.removeAtKey(defaultFieldName);
                    }
                } 
                
                var realFieldName       = overrideFieldName || defaultFieldName;
                
                if (!fields.containsKey(realFieldName)) {
                    // we create a new copy of the `defaultField` using possibly new name 
                    fields.add(new Ext.data.Field(Ext.applyIf({ name : realFieldName }, defaultField)));
                }
                
                var capitalizedDefaultName  = Ext.String.capitalize(defaultFieldName);
                
                // don't overwrite `getId` method
                if (capitalizedDefaultName != 'Id') {
                    var getter              = 'get' + capitalizedDefaultName;
                    var setter              = 'set' + capitalizedDefaultName;
...
Ext.define('Sch.model.Range', {
    ...
    */
    ...
    /**
    * @cfg {String} clsField The name of the field that holds the event "class" value (usually corresponds to CSS class). Defaults to "Cls".
    */
    clsField        : 'Cls',
 ...
 * 
 * This is a class holding all the resources to be rendered into a SchedulerPanel. Its a subclass of "Ext.data.Store" - a store with linear data presentation.
 */
...
Ext.define('Sch.mixin.Lockable', {
                ...
                if (features[i].isFeature) {
                    if (features[i] instanceof Ext.ClassManager.get(o)) {
                        if (map[o] === 'locked' || map[o] === 'both') {
                            ...
                            if (features[i] instanceof Ext.grid.feature.Grouping) {
                                me.lockedGrouping = features[i];
                            }
                        }
                        if (map[o] === 'normal' || map[o] === 'both') {
                            var copy = new features[i].self();
                            normal.push(copy);
                            if (copy instanceof Ext.grid.feature.Grouping) {
                                me.normalGrouping = copy;
                            }
                        }
    ...
    injectLockable: function () {
        ...
        var processedFeatures = this.processFeatures(this.features),
            ...
            eventSelModel = me.getEventSelectionModel ? me.getEventSelectionModel() : me.getSelectionModel();
        me.lockedGridConfig = me.lockedGridConfig || {};
        me.normalGridConfig = me.schedulerConfig || {};

        var lockedGrid = me.lockedGridConfig,
            normalGrid = me.normalGridConfig;

        Ext.apply(me.lockedGridConfig, {
            enableLocking: false,
            lockable: false,
            useArrows: true,
        ...
        normalGrid.viewConfig.enableAnimations = lockedGrid.viewConfig.enableAnimations = false;
        ...
        } else if (me.lockedGrouping) {
            // Sync group expand/collapse for the two views.
            lockedView.on({
                groupexpand: this.onLockedViewGroupExpand,
                groupcollapse: this.onLockedViewGroupCollapse,
                scope: this
            });
            normalView.on({
                groupexpand: this.onNormalViewGroupExpand,
                groupcollapse: this.onNormalViewGroupCollapse,
                scope: this
            });

            // Needed for Lockable to work with Grouping patch
            lockedView.on({
        ...
        if (me.syncRowHeight) {
            normalView.on({
                // required for tree
                itemadd: me.onNormalViewItemAdd,
                scope: me
            });

            // PATCH broken implementation broken for IE9
            if (Ext.isIE9 && Ext.isStrict) {
                me.onNormalViewRefresh = function () {
                ...
                me.onNormalViewItemUpdate = function (record, index, node) {
                    ...
                    me.normalHeights[index] = row.offsetHeight;
    ...
    setupLockableTree: function () {
        ...
        treeStore.on('cascade', function (treeStore, context) {
            ...
            if (context.nbrAffected > 0) {
                me.getSchedulingView().refreshKeepingScroll();
            }
        });

        // re-using the NodeStore for the normal grid
        lockedView.store.destroyStore();
        lockedView.bindStore(normalView.store);

        if (isBuffered && me.normalGrid.verticalScroller) {
            me.normalGrid.verticalScroller.store = lockedView.store;
        }
    },
    ...
    onNormalViewItemAdd: function (records, index, nodes) {
    ...
    onNormalViewRefresh: function () {
        ...
        for (; i < ln; i++) {
            var h = rowEls[i].style.height;
            if (h) {
                me.normalHeights[i] = h.substring(0, h.length - 2);
            }
        }
        me.syncRowHeights();
    },
    ...
    createSpacer: function () {
        ...
        if (this.lockedGrid.rendered && this.normalGrid.rendered) {
            this.callParent(arguments);
        }
    },

    //    // TODO check if this is still required in 4.2
    ...
    //                var normalHeaderHeight  = normalHeaderCt.getHeight();
    //        
    //                innerEl.setHeight(normalHeaderHeight);
    //                        
    //                lockedHeaderCt.items.each(function (container) {
    //                    container.setPadding(normalHeaderHeight);
    //                });
    //            }
    //        }
    //    },
    ...
    // Sync group expand/collapse for the two views.
    onLockedViewGroupExpand: function (v, groupName) {
        this.normalGrid.view.suspendEvents();
        this.normalGrouping.expand(groupName);
        this.normalGrid.view.resumeEvents();
    },

    // Sync group expand/collapse for the two views.
    onLockedViewGroupCollapse: function (v, groupName) {
        this.normalGrid.view.suspendEvents();
        this.normalGrouping.collapse(groupName);
        this.normalGrid.view.resumeEvents();
    },

    // Sync group expand/collapse for the two views.
    onNormalViewGroupExpand: function (v, groupName) {
        this.lockedGrid.view.suspendEvents();
        this.lockedGrouping.expand(groupName);
        this.lockedGrid.view.resumeEvents();
    },

    // Sync group expand/collapse for the two views.
    onNormalViewGroupCollapse: function (v, groupName) {
        this.lockedGrid.view.suspendEvents();
        this.lockedGrouping.collapse(groupName);
        this.lockedGrid.view.resumeEvents();
    },
    ...
    */
    ...
    calculateTimeColumnWidth: function (proposedTimeColumnWidth) {
        ...
        if (!this.forceFit || ratio < 1) {
            width = Math.round(Math.max(1, Math[this.forceFit ? 'floor' : 'round'](ratio * width)) / ratio);
        }
    ...
    props: {
        ...
        */
        ...
        collectRowData: function (rowData, resourceRecord, index) {
            var resourceEvents = resourceRecord.getEvents();
...
Ext.define('Gnt.model.CalendarDay', {
    ...
    setDate : function (date) {
        this.set(this.dateField, Ext.Date.clearTime(date, true));
    },
    ...
    // decode to [ { startDate : Date, endDate : Date } ]
    getAvailability : function (asString) {
        ...
        return this.availability = parsed
    },
    ...
    verifyAvailability : function (intervals) {
        ...
        Ext.Array.each(intervals, function (interval) {
            if (interval.startTime > interval.endTime) throw "Start time is greater than end time"
        })
        ...
        for (var i = 1; i < intervals.length; i++) {
            ...
            if (prev.endTime > curr.startTime) throw "Availability intervals should not intersect"
        }
    },
...
Ext.define('Gnt.model.Assignment', {
    ...
    getResourceName : function() {
        return this.stores[ 0 ].getResourceStore().getById(this.getResourceId()).getName();
    },

    
    // removed assignment will not have a "this.stores" so we are providing reserve way to get the task
...
This class represent a single Dependency in your gantt chart. It is a subclass of the {@link Ext.data.Model}.
Please refer to {@link Ext.data.Model} documentation to become familar with the base interface of this class.
...
Ext.define('Gnt.model.Resource', {
    ...
    forEachAvailabilityIntervalWithTasks : function (options, func, scope) {
        ...
        while (cursorDate < endDate) {
            ...
            for (var i = 0; i < tasksCalendars.length; i++) {
        ...
        for (var i = 0; i < pointTimes.length - 1; i++) {
...
Ext.define('Gnt.model.Task', {
    ...
    * This field is deprecated in favor of "schedulingModeField". To specify that task is manually scheduled set the value of "schedulingModeField" to "Manual"
    */ 
    ...
    * 
    * XXX improve and describe all scheduling modes in great details
    * 
    * Default mode is "Normal" meaning the task will be scheduled based only on information about it start/end dates and project calendar.
    * 
    * 'FixedDuration'
    * 
    * 'EffortDriven'
    ...
    * 'DynamicAssignment'
    * 
    * The "Manual" mode disables any additional calculations and task will be scheduled based only on information about it start/end dates.
    */ 
    ...
    */
    ...
    normalize: function () {
        ...
        if (duration == null && startDate && endDate) {
            this.data[ this.durationField ] = this.calculateDuration(startDate, endDate, durationUnit);
        }

        if ((schedulingMode == 'Normal' || this.isManuallyScheduled()) && endDate == null && startDate && duration) {
            this.data[ this.endDateField ] = this.calculateEndDate(startDate, duration, durationUnit);
        }
        
        if (schedulingMode == 'FixedDuration' && effort == null && startDate && endDate) {
            this.data[ this.effortField ] = this.calculateEffort(startDate, endDate, effortUnit);
        }
        
        if (schedulingMode == 'EffortDriven' && endDate == null && startDate && effort) {
            this.data[ this.endDateField ]  = this.calculateEffortDrivenEndDate(startDate, effort, effortUnit);
            
            // for "effortDriven" task user can only provide StartDate and Effort - thats all we need
            if (duration == null) {
                this.data[ this.durationField ] = this.calculateDuration(startDate, this.data[ this.endDateField ], durationUnit);
            }
        }
...
//        if (value == 'Normal' || value == 'Manual') {}
    ...
    * Returns the effort of the task expressed in the unit passed as the only parameter (or as specified by the EffortUnit for the task). 
    * 
    * @param {String} unit Unit to return return the duration in. Defaults to the `EffortUnit` field of this task
    * 
    * @return {Number} duration
    */
    ...
    */
    ...
    forEachAvailabilityIntervalWithResources : function (options, func, scope) {
        ...
        while (!endDate || cursorDate < endDate) {
            ...
            // using "for" instead of "each" should be blazing fast! :)
            for (var k = 0; k < taskIntervals.length; k++) {
                var interval            = taskIntervals[ k ]
                var intervalStartDate   = interval.startDate - 0
                var intervalEndDate     = interval.endDate - 0
            ...
            // using "for" instead of "each" should be blazing fast! :)
            for (var i = 0; i < resourcesCalendars.length; i++) {
                var resourceIntervals       = resourcesCalendars[ i ].getAvailabilityIntervalsFor(cursorDate)
                
                // using "for" instead of "each" should be blazing fast! :)
                for (var k = 0; k < resourceIntervals.length; k++) {
                    var interval            = resourceIntervals[ k ]
                    var intervalStartDate   = interval.startDate - 0
                    var intervalEndDate     = interval.endDate - 0
    ...
    */
    ...
    /**
    * Returns the effor unit of the task.  
    * @return {String} the effor unit
    */
    ...
    * 
    * Updates the effor unit of the task.  
    * 
    * @param {String} unit New effor unit
    * @return {String} the effor unit
    */
    ...
    */
    ...
    assign : function (resource, units) {
        ...
        assignmentStore.add(Ext.create('Gnt.model.Assignment', {
            'TaskId'        : id,
...
Ext.define('Gnt.data.Calendar', {
    ...
    // default availability times for a week
    weekAvailability                : null,
    
    // reference to a parent Calendar
    parent                          : null,
    ...
    statics: {
        getCalendar: function (id) {
            ...
            return Ext.StoreManager.lookup('GNT_CALENDAR:' + id);
        }       
    },   
    ...
    constructor : function (config) {
        ...
        // if "parent" is provided then, by default, weekAvailability is assumed to be []
        // of course if user provides it explicitly, that takes most precedence
        // TODO add day names "Sunday/Monday" etc
        this.weekAvailability = config.weekAvailability || parent ? [] : [
            // 0 - Sunday
            this.weekendsAreWorkdays ? 
                new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '00:00-24:00' ], IsWorkingDay : true })
                    :
                new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [] }),
            // 1 - Monday, etc
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '00:00-24:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '00:00-24:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '00:00-24:00' ], IsWorkingDay : true }),
    ...
    getCalendarDay : function (timeDate) {
        ...
        return this.getDefaultCalendarDay(timeDate.getDay())
    },
    ...
    // TODO - cache
    getAvailabilityIntervalsFor : function (date) {
        return this.getCalendarDay(date).getAvailabilityIntervalsFor(date)
    },
    
    
    intersectAvailabilities : function (intervals1, intervals2) {
        throw "yo"
    }
});
/**
...
Ext.define('Gnt.data.Calendar.BusinessTime', {
    ...
    constructor : function (config) {
        ...
        // TODO add day names "Sunday/Monday" etc
        this.weekAvailability = config.weekAvailability || this.parent ? [] : [
            // 0 - Sunday
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : []  }),
            // 1 - Monday, etc
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '08:00-12:00', '13:00-17:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '08:00-12:00', '13:00-17:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '08:00-12:00', '13:00-17:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '08:00-12:00', '13:00-17:00' ], IsWorkingDay : true }),
            new Gnt.model.CalendarDay({ Date : new Date(), Availability    : [ '08:00-12:00', '13:00-17:00' ], IsWorkingDay : true }),
...
//        }
    ...
    */
    ...
    onTaskUpdated: function (store, task, operation) {
        ...
        if (!this.cascading && 
            (operation === Ext.data.Model.EDIT || operation === Ext.data.Model.REJECT) &&
            (!prev || task.startDateField in prev || task.endDateField in prev)) {
...
//        me.resumeEvents();
    ...
    onAssignmentMutation : function (assignmentStore, assignments) {
        ...
        Ext.each(assignments, function (assignment) {
            assignment.getTask(me).onAssignmentMutation(assignment)
        });
    },
...
A class representing the dependencies between the tasks in the {@link Gnt.data.TaskStore}.
...
//            delay       : (this.taskStore.buffered && Ext.isIE) ? 1 : 0 // IE screws up drawing if refresh happens too fast
    ...
    */
    ...
    onTaskUpdated: function (store, task, operation) {
        if (operation != Ext.data.Model.COMMIT && (!task.previous || task.startDateField in task.previous || task.endDateField in task.previous)) {
        ...
        if (this.enableBaseline) {
            ...
            if (taskBaselineStart && taskBaselineEnd) {
                var endsOutsideView = taskBaselineEnd > viewEnd,
                    ...
                    isMileStone = taskModel.isMilestone(),
                    baseTpl = isMileStone ? this.baselineMilestoneTemplate : (taskModel.isLeaf() ? this.baselineTaskTemplate : this.baselineParentTaskTemplate),
    ...
    */
    ...
    initStores : function() {     
        ...
        if (!(this.dependencyStore instanceof Gnt.data.DependencyStore)) {
            ...
            this.dependencyStore.self.mixin('dependencyStoreAdaptions', Gnt.data.DependencyStore);
            this.dependencyStore.init();
        }
...
Ext.define("Gnt.column.StartDate", {
    ...
    onBeforeTreeEdit : function (context) {
        ...
        if (context.column == this) {
            ...
            this.field.startValue       = context.value = Ext.Date.clearTime(context.value);
        }
    },
...
Ext.define('Gnt.widget.AssignmentCellEditor', {
    ...
    onSelect : function(field, selections) {
        ...
        aStore.each(function (assignment) {
            //   assignemnt is for our task       | not phantom |       was unchecked
