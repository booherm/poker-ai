 ...
 *    2012-01-06 updated to have searcher, and hidden value by Paul Schroeder
 *
 * @constructor
...
Ext.define('Ext.ux.form.MultiSelect', {
	...
	initComponent: function(){
		...
		var objectS = me.setupItems();
		Ext.apply(me, objectS);


		me.callParent();
		me.initField();
		if(me.panel)
			me.panel.addEvents('drop');    
	},

	setupItems: function() {
		var me = this;

		var style = "font-size:11px;padding-top:1px;padding-bottom:1px;border-width:0px;";
		if(me.fixedRowHeight){
			style += "height:" + me.fixedRowHeight + "px;";
		}
		...
		me.boundList = Ext.create('Ext.view.BoundList', {
			...
			tpl:          Ext.create('Ext.XTemplate',
				'<ul><tpl for=".">',
					'<li style="' + style + '" unselectable="on" role="option" class="' + Ext.baseCSSPrefix + 'boundlist-item' + '">{' + me.displayField + '}</li>',
				'</tpl></ul>'
			)
		});
		me.boundList.selectedItemCls = "x-boundlist-selected-multiselect";
		me.boundList.overItemCls = "x-boundlist-over-multiselect";

		me.boundList.getSelectionModel().on('selectionchange', me.onSelectChange, me);

		var headerItems = [];
		if(me.showSearchBox || me.searchBoxConfig)
		{
			me.searchBoxConfig = me.searchBoxConfig ? {
				width:    me.searchBoxConfig.width,
				plugins:  me.searchBoxConfig.plugins,
				onSearch: function(field, newValue){
					...
					if(Ext.isString(newValue))
					{
						me.store.filterBy(function(record, id){
							...
							if(Ext.isString(value) && (value.toUpperCase().indexOf(newValue.toUpperCase()) != -1))
								return true;
							return false;
						});
					}
			...
			headerItems.push(new Ext.Img({
				src: "extjs/4.0/sms/icons/fam/magnifier.png",
				height: 16,
				width:  16,
				margin: "0 3 0 0"
			}));
			me.searchBox = new Ext.form.TextField({
				width: me.searchBoxConfig.width ? me.searchBoxConfig.width : 200,
				plugins: me.searchBoxConfig.plugins,
				height: 18,
				listeners: {
					change: me.searchBoxConfig.onSearch
				}
			});
			headerItems.push(me.searchBox);
		}
		...
		var panelConfig = {
			...
			tbar: me.tbar,
			layout: "fit",
			style:  Ext.isGecko ? "position:relative !important;" : "",
			items: [
				me.boundList
			],
			listeners: {
				afterrender: function(thePanel){
					if(thePanel.header)
						thePanel.header.add(headerItems);
				}
			}
		};

		//IE8 is the only browser that renders funny if using border layout
		if(!Ext.isIE8)
			panelConfig.region = "center";

		me.panel = Ext.create("Ext.panel.Panel", panelConfig);

		return {
			panel:  me.panel,
			layout: Ext.isIE8 ? 'fit' : "border",//fix for rendering issue
			items: [
				me.panel
			]
		};
	...
	sortInactiveRecords: function(store) {
		var me = this;
		if(!store)
			store = me.store;

		var records = me.inactiveOptionsStore.getRange();
		me.inactiveOptionsStore.remove(records);
		store.loadRecords(records, {addRecords: true});
		var inactiveRecords = store.queryBy(me.inactiveOptionsFunction).getRange();
		store.remove(inactiveRecords);
		me.inactiveOptionsStore.loadRecords(inactiveRecords, {addRecords: true});
	},
	...
	getErrors : function(value) {
		...
		if(!errors.length && me.validator)
		{
			var result = me.validator(value);
			if(result !== true)
				errors.push(result);
		}
		return errors;
	...
	onBindStore: function(store){
		...
		if(me.inactiveOptionsFunction && !me.inactiveOptionsStore)
		{
			me.inactiveOptionsStore = Ext.create('Ext.data.Store', {model: store.model});
			me.sortInactiveRecords(store);
		}
